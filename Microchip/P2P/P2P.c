    ...
    void DumpConnection(INPUT BYTE index)
    {
        ...
        if( index < CONNECTION_SIZE )
        {
            ...
            if( ConnectionTable[index].status.bits.isValid )
            {
                ...
                for(i = 0; i < 8; i++)
                {
                    if((i < MY_ADDRESS_LENGTH) && ConnectionTable[index].Address[MY_ADDRESS_LENGTH-1-i])
                    {
                        ConsolePut( ConnectionTable[index].Address[MY_ADDRESS_LENGTH-1-i] );
                    }
                    else
                    {
                        Printf(" ");
                ...
                Printf("            ");
                    ...
                    for(i = 0; i < ADDITIONAL_NODE_ID_SIZE; i++)
                    {
                        ConsolePut( ConnectionTable[index].PeerInfo[i] );
                    }
                #endif
                Printf("\r\n");
            }
