...
This is the Mass Storage Class driver file for a USB Host device.  This file
should be used in a project with USBHost.c to provided the USB hardware
interface.
...
To interface with USBHost.c, the routine USBMSDClientInitialize() should be
...
declared in USBConfig.h.

This driver can be configured to use transfer events from USBHost.c.  Transfer
events require more RAM and ROM than polling, but it cuts down on the
required polling of the various USBxxxTasks functions.  For this class,
USBHostMSDTasks() does not have to be called at all if transfer events from
USBHost.c are used.  However, USBHostTasks() still must be called to provide
attach, enumeration, and detach services.  If transfer events from USBHost.c
...
mass storage transfer status is going to be polled, USB_MSD_ENABLE_TRANSFER_EVENT
...
#ifdef USB_MAX_MASS_STORAGE_DEVICES
    #define MAX_DEVICES USB_MAX_MASS_STORAGE_DEVICES
#else
    #define MAX_DEVICES 1   // The maximum number of attached devices this
                            // class driver can support.  Currently this must
                            // be set to 1.
#endif
...
USB_MSD_DEVICE_INFO         deviceInfoMSD[MAX_DEVICES] __attribute__ ((aligned));
...
BYTE USBHostMSDDeviceStatus( BYTE deviceAddress )
{
    ...
    for (i=0; (i<MAX_DEVICES) && (deviceInfoMSD[i].deviceAddress != deviceAddress); i++);
    if (i == MAX_DEVICES)
    {
        return USB_MSD_DEVICE_NOT_FOUND;
    }
...
Notes:          The presence of a return code is for similarity with other
...
BYTE USBHostMSDInit( void )
{
    ...
    {
        memset( &deviceInfoMSD[i], 0x00, sizeof(USB_MSD_DEVICE_INFO) );
    }
...
BOOL USBHostMSDTransferIsComplete( BYTE deviceAddress, BYTE *errorCode, DWORD *byteCount )
{
   ...
   // Find the correct device.
    for (i=0; (i<MAX_DEVICES) && (deviceInfoMSD[i].deviceAddress != deviceAddress); i++);
    if ((i == MAX_DEVICES) || (deviceInfoMSD[i].state == STATE_DETACHED))
    {
        *errorCode = USB_MSD_DEVICE_NOT_FOUND;
        *byteCount = 0;
        return TRUE;
...
/*******************************************************************************
Function:       BOOL USBMSDClientInitialize( BYTE address, DWORD flags )

Precondition:   None

Overview:       This function is called by USBHost.c when the USB device is
                being enumerated.  For a mass storage device, we need to make
                sure that we have room for a new device, and that the device has
...
Return:         TRUE   - We can support the device.
...
BOOL USBMSDClientInitialize( BYTE address, DWORD flags )
{
    BYTE   *descriptor;
    BYTE    device;
    BYTE    errorCode;
    ...
    for (device = 0; (device < MAX_DEVICES) && (deviceInfoMSD[device].deviceAddress != 0); device++);
...
/*******************************************************************************
Function:       BOOL USBMSDClientEventHandler( BYTE address, USB_EVENT event,
                        void *data, DWORD size )

Precondition:   The device has been initialized.

Overview:       This function is called by USBHost.c when various events occur
                in the USB Host layer.
...
BOOL USBMSDClientEventHandler( BYTE address, USB_EVENT event, void *data, DWORD size )
{
    ...
    switch (event)
    {
        ...
        case EVENT_DETACH:           // USB cable has been detached (data: BYTE, address of device)
            ...
            for (i=0; (i<MAX_DEVICES) && (deviceInfoMSD[i].deviceAddress != address); i++);
            if (i < MAX_DEVICES)
            {
        ...
        case EVENT_HUB_ATTACH:       // USB hub has been attached
            usbMediaInterfaceTable.EventHandler( address, EVENT_HUB_ATTACH, NULL, 0 );
            return TRUE;
            break;
...
/*******************************************************************************
Function:       DWORD _USBHostMSD_GetNextTag( void )

Precondition:   None

Overview:       Each bulk transfer has a transfer ID that is present in the CBW
                and must match in the CWS.  This function gets the next ID
                number for a transfer.

Input:          None - None

Output:         None - None

Return:         DWORD - Tag to use in the next CBW
...
/*******************************************************************************
Function:       void _USBHostMSD_ResetStateJump( BYTE i )

Precondition:   The device information must be in the deviceInfoMSD array.

Overview:       This function determines which portion of the reset processing
                needs to be executed next and jumps to that state.

Input:          BYTE i  - Index into the deviceInfoMSD structure for the device
                            to reset.
