...
** 03/10/10 v3.0 LDJ skinny version
*/

#define STATIC_ALLOC        // usa allocazione statica buffer
...
unsigned fseekM( unsigned size, MFILE *fp)
// fp       pointer to MFILE structure
// count    number of bytes to skip
// returns  0 if successfull
{
    MEDIA * mda = fp->mda;      
    unsigned count=size;        // counts bytes to transfer
    unsigned len, cluster;          
    ...
    // 2. check if file is large enough 
    if ( ( fp->seek + count) >= fp->size)
    {
        FError = FE_EOF;  // reached the end
        return -1;          // return error
    }
    
    // 3. check if data is already in the current buffer
    if ( fp->top - fp->pos >= count )
    {   // use it all up
        fp->pos += count;
        fp->seek += count;
        return 0;           // success
    ...
    // 4. take it all 
    count -= fp->top - fp->pos;
    fp->seek += fp->top - fp->pos;
    fp->pos = 0;
    fp->sec++;
    
    // check if a new cluster must be started
    if ( fp->sec == mda->sxc)
    {
        fp->sec = 0;
        // get new cluster 
    ...
    // 4.1 compute reminder in cluster
    cluster = ( ( mda->sxc - fp->sec) * 512);
    
    // 5. check if current cluster does not contain enough
    while ( cluster < count)
    { 
        fp->seek += cluster;
        count -= cluster;        

        // 5.1 advance by as many clusters as required
        if ( !nextFAT( fp, 1))
            return -1;      // error
        cluster = ( mda->sxc * 512);        // one full cluster
        fp->sec = 0;                        // start with first sector
    }        
    
    // 6. enough data is available in cluster, find required sector    
    fp->sec += count / 512;
    fp->pos = count % 512;
    fp->seek += count;
        
    // 7. load a sector of data
    if ( !readDATA( fp))
        return -1;      // report error

    // 8. determine how much data is inside buffer
    if ( fp->size-fp->seek < 512)
        fp->top = fp->size - fp->seek;
    else
        fp->top = 512;
    ...
    // 9. return number of bytes actually transferred 
    return 0;
...
//-------------------------------------------------------------
// Scans the media and compiles a list of files 
